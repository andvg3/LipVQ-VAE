...
def make_generator_helper(args):
    algo_name_short = "icl_mamba"

    generator = get_generator(
        algo_name="icl_mamba",
        config_file=os.path.join(base_path, "robomimic/exps/templates/icl_mamba.json"),
        args=args,
    ...
    generator.add_param(
        ...
        values_and_names=[
            ...
            (
                get_robocasa_ds(
                    "single_stage", src="human", eval=[], filter_key="50_demos"
                ),
                "human-50",
            ),  # training on human datasets
            (
                get_robocasa_ds(
                    "single_stage", src="mg", eval=[], filter_key="3000_demos"
                ),
                "mg-3000",
            ),  # training on MimicGen datasets
            # composite tasks
            (
                get_robocasa_ds("ArrangeVegetables", filter_key="50_demos"),
                "ArrangeVegetables",
            ),
            (
                get_robocasa_ds("MicrowaveThawing", filter_key="50_demos"),
                "MicrowaveThawing",
            ),
