...
from robomimic.algo.mcr import load_mcr
from robomimic.algo import register_algo_factory_func, PolicyAlgo


@register_algo_factory_func("mcr")
def algo_config_to_class(algo_config):
    """
    Maps algo config to the MCR algo class to instantiate, along with additional algo kwargs.
    ...
    if gaussian_enabled:
        ...
        if rnn_enabled:
            ...
            algo_class, algo_kwargs = MCR_Transformer_GMM, {}
        else:
...
class MCR_Transformer_GMM(BC_Transformer):
    """
    MCR training with a Transformer GMM policy.
    """
    ...
    def _create_networks(self):
        ...
        self.mcr_model = load_mcr(ckpt_path="expdata/robocasa/mcr/mcr_resnet50.pth").to(
            self.device
        )
        self.nets = nn.ModuleDict()
        self.nets["policy"] = PolicyNets.MCRGMMActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            num_modes=self.algo_config.gmm.num_modes,
