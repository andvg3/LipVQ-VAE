...
@register_algo_factory_func("icl_mamba")
def algo_config_to_class(algo_config):
    """
    Maps algo config to the ICL Mamba algo class to instantiate, along with additional algo kwargs.
    ...
    mamba_enabled = algo_config.mamba.enabled
    ...
    if gaussian_enabled:
        ...
        if rnn_enabled:
            ...
            algo_class, algo_kwargs = ICLMamba_GMM, {}
        else:
        ...
        if rnn_enabled:
            ...
            algo_class, algo_kwargs = ICLVAE, {}
    else:
        if rnn_enabled:
            algo_class, algo_kwargs = ICLRNN, {}
        elif mamba_enabled:
            algo_class, algo_kwargs = ICLMamba, {}
        else:
...
class ICLMamba(ICL):
    """
    ICL training with a Mamba policy.
    """
    ...
    def _create_networks(self):
        ...
        Creates networks and places them into @self.nets.
        """
        assert self.algo_config.mamba.enabled

        self.nets = nn.ModuleDict()
        self.nets["policy"] = PolicyNets.MambaActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            encoder_kwargs=ObsUtils.obs_encoder_kwargs_from_config(
            ...
            **BaseNets.mamba_args_from_config(self.algo_config.mamba),
        )
    ...
    def _set_params_from_config(self):
        ...
        """
        self.context_length = self.algo_config.mamba.context_length
        ...
        self.ln_act_enabled = self.algo_config.mamba.ln_act_enabled
    ...
    def _forward_training(self, batch, epoch=None):
        """
        Internal helper function for ICLMamba algo class. Compute forward pass
        and return network outputs in @predictions dict.
        ...
        Returns:
            predictions (dict): dictionary containing network outputs
        """
        # ensure that mamba context length is consistent with temporal dimension of observations
        TensorUtils.assert_size_at_dim(
            batch["obs"],
            size=(self.context_length),
            dim=1,
            msg="Error: expect temporal dimension of obs batch to match mamba context length {}".format(
                self.context_length
            ),
        )
    ...
    def get_action(self, obs_dict, context_batch, goal_dict=None):
        ...
        if self.supervise_all_steps:
            if self.algo_config.mamba.pred_future_acs:
                output = output[:, 0, :]
            else:
                output = output[:, -1, :]
        else:
...
class ICLMamba_GMM(ICLMamba):
    """
    ICL training with a Mamba GMM policy.
    """
    ...
    def _create_networks(self):
        ...
        self.nets = nn.ModuleDict()
        self.nets["policy"] = PolicyNets.ICLMambaGMMActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            num_modes=self.algo_config.gmm.num_modes,
    ...
    def _compute_losses(self, predictions, batch):
        """
        Internal helper function for ICLMamba_GMM algo class. Compute losses based on
        network outputs in @predictions dict, using reference labels in @batch.
        Args:
            predictions (dict): dictionary containing network outputs, from @_forward_training
            batch (dict): dictionary with torch.Tensors sampled
