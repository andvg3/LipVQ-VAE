...
class ICLMambaConfig(BaseConfig):
    ALGO_NAME = "icl_mamba"
    ...
    def train_config(self):
        ...
        """
        super(ICLMambaConfig, self).train_config()
    ...
    def algo_config(self):
        ...
        # Mamba policy settings
        self.algo.mamba.enabled = False  # whether to train transformer policy
        self.algo.mamba.context_length = 10  # length of (s, a) seqeunces to feed to transformer - should usually match train.frame_stack
        self.algo.mamba.embed_dim = 512  # dimension for embeddings used by transformer
        self.algo.mamba.num_layers = 6  # number of transformer blocks to stack
        self.algo.mamba.num_heads = 8  # number of attention heads for each transformer block (should divide embed_dim evenly)
        self.algo.mamba.emb_dropout = (
            0.1  # dropout probability for embedding inputs in transformer
        )
        self.algo.mamba.attn_dropout = (
            0.1  # dropout probability for attention outputs for each transformer block
        )
        self.algo.mamba.block_output_dropout = (
            0.1  # dropout probability for final outputs for each transformer block
        )
        self.algo.mamba.sinusoidal_embedding = (
        ...
        self.algo.mamba.bin_enabled = False
        self.algo.mamba.vq_vae_enabled = False
        self.algo.mamba.ln_act_enabled = True
        ...
        self.algo.mamba.pred_future_acs = False  # shift action prediction forward to predict future actions instead of past actions
        self.algo.mamba.causal = True  # whether the transformer is causal
