...
class Tokenizer:
    def __init__(self, algo, vocab_size):
        ...
        self.char_index_map = {
            char: str(index) for index, char in enumerate(self.alphabet)
        }
        ...
        if algo == "wordpiece":
            self.tokenizer = tokenizers.Tokenizer(
                Model(unk_token="[UNK]", max_input_chars_per_word=100000)
            )
    ...
    def detextualize(self, text):
        ...
        decoded_text = "".join(
            [
                self.char_index_map[char] if char in self.char_index_map else char
                for char in text
            ]
        )
...
if __name__ == "__main__":
    action_tokenizer = Tokenizer("bpe", 2048)
    print(action_tokenizer.textualize([2, 2]))

    # Example of using self.train()
    sample_corpus = [
        [1, 2, 3],
        [1, 4, 5, 6],
        [7, 8, 9],
    ]  # Example corpus (list of lists of integers)
    action_tokenizer.train(
        sample_corpus, min_frequency=1, max_token_length=10, verbose=True
    )

    # Example of encoding and decoding after training
    encoded_ids = action_tokenizer.encode([1, 2, 3, 5, -1, 7], verbose=True)
    decoded_ids = action_tokenizer.decode(encoded_ids, verbose=True)
