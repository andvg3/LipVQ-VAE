...
class MCRTransformerActorNetwork(MIMO_MCR_Transformer):
    """
    An Transformer policy network that predicts actions from observation sequences (assumed to be frame stacked
    from previous observations) and possible from previous actions as well (in an autoregressive manner).
    """
    ...
    def __init__(
        ...
        output_shapes = self._get_output_shapes()
        super(MCRTransformerActorNetwork, self).__init__(
            input_obs_group_shapes=observation_group_shapes,
            output_shapes=output_shapes,
            transformer_embed_dim=transformer_embed_dim,
            transformer_num_layers=transformer_num_layers,
...
class ICLTransformerActorNetwork(ICL_MIMO_Transformer):
    ...
    def __init__(
        ...
        super(ICLTransformerActorNetwork, self).__init__(
            ...
            transformer_activation=transformer_activation,
            transformer_nn_parameter_for_timesteps=transformer_nn_parameter_for_timesteps,
            transformer_fast_enabled=transformer_fast_enabled,
            transformer_bin_enabled=transformer_bin_enabled,
            transformer_vq_vae_enabled=transformer_vq_vae_enabled,
            transformer_ln_act_enabled=transformer_ln_act_enabled,
            encoder_kwargs=encoder_kwargs,
        )
...
class ICLMambaActorNetwork(ICL_MIMO_Mamba):
    ...
    def __init__(
        ...
        super(ICLMambaActorNetwork, self).__init__(
            ...
            mamba_bin_enabled=mamba_bin_enabled,
            mamba_vq_vae_enabled=mamba_vq_vae_enabled,
            mamba_ln_act_enabled=mamba_ln_act_enabled,
            encoder_kwargs=encoder_kwargs,
        )
...
class MCRGMMActorNetwork(MCRTransformerActorNetwork):
    """
    A Transformer GMM policy network that predicts sequences of action distributions from observation
    sequences (assumed to be frame stacked from previous observations).
    """
    ...
    def __init__(
        ...
        transformer_bin_enabled=False,
        transformer_vq_vae_enabled=False,
        transformer_ln_act_enabled=True,
        num_modes=5,
        min_std=0.01,
        std_activation="softplus",
        low_noise_eval=True,
        ...
        MIMO_MCR_Transformer.set_mcr_model(mcr_model)

        super(MCRGMMActorNetwork, self).__init__(
            obs_shapes=obs_shapes,
            ac_dim=ac_dim,
            transformer_embed_dim=transformer_embed_dim,
            transformer_num_layers=transformer_num_layers,
    ...
    def forward_train(
        ...
        outputs = MIMO_MCR_Transformer.forward(self, **forward_kwargs)
