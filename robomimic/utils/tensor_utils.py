...
def mcr_time_distributed(
    ...
    **kwargs
    ...
    # with torch.no_grad():
    #     embedding = cls.mcr(preprocessed_image.cuda())
...
def icl_time_distributed(
    ...
    obs = reshape_dimensions(
        obs, begin_axis=0, end_axis=0, target_dims=(batch_size, seq_len)
    )
    context_obs = reshape_dimensions(
        context_obs, begin_axis=0, end_axis=0, target_dims=(batch_size, seq_len)
    )
    context_actions = reshape_dimensions(
        context_actions, begin_axis=0, end_axis=0, target_dims=(batch_size, seq_len)
    )
